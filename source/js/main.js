(function () {
  ...
  telInput.addEventListener(`focus`, function () {
    const telMask = window.mask.imask(telInput, maskOptions);
    telMask.cursorPos = 3;
    telInput.value = `+7(`;
  });
  ...
  const openModal = () => {
    ...
    const firstName = document.querySelector(`.callback-modal .callback-modal__first-name input`);
    firstName.focus();
  };
  ...
  callbackButton.addEventListener(`click`, function () {
    ...
    callbackTelInput.addEventListener(`focus`, function () {
      const callbackTelMask = window.mask.imask(callbackTelInput, maskOptions);
  ...
  const CLOSE_SITE_MAP = `Закрыть разделы сайта`;
